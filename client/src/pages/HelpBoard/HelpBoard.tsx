...
interface HelpBoardData {
  helpBoards: HelpBoard[];
}

interface HelpBoardVars {
  title: string;
  description: string;
  date: string;
  status: string;
...
export default function HelpBoard() {
  const { loading , data} = useQuery(FIND_ALL_POST);
  const helpBoards = data?.findAllPosts || [];

  return (
    <div>
      <Typography variant="h4">Help Board</Typography>
      {loading && <Typography>Loading...</Typography>}
      {helpBoards.map((helpBoard: HelpBoard) => (
...
// export default function HelpBoard() {
//   useAuth();
//   const { loading, error, data, refetch } = useQuery<HelpBoardData, HelpBoardVars>(
//     FIND_ALL_POST
//   );
...
//   const handleAdd = () => {
//     addHelpBoard({
//       variables: { input: { title, description, date, status, createdBy, completedBy } }
//     }).then(() => {
//       setTitle('');
//       setDescription('');
//       setDate('');
...
//               </CardContent>
//               <CardActions>
//                 <Button onClick={() => handleRemove(helpBoard._id)}>Remove</Button>
//               </CardActions>
//             </Card>
//           ))}
//         </div>
